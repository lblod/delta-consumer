import { querySudo as query, updateSudo as update } from './sparql';
import { sparqlEscapeDateTime, sparqlEscapeUri, sparqlEscapeString } from 'mu';
import { PREFIXES } from './constants';
import DeltaFile from "./delta-file";

const INVALID_DATATYPES = (process.env.INVALID_DATATYPES || '').split(',').map(e => e.trim());
...
export function toTermObjectArray(triples) {
  const escape = function (rdfTerm) {
    ...
    if (type === 'uri') {
      ...
      if (datatype && !INVALID_DATATYPES.includes(datatype))
