...
import { toTermObjectArray, prepareStatements, updateWithRecover } from './utils';
...
async function findSubjectTypes(subject) {
  try {
    const response = await querySudo(subjectTypeQuery(LANDING_ZONE_GRAPH, subject), {}, { sparqlEndpoint: LANDING_ZONE_DATABASE_ENDPOINT, mayRetry: true });
...
async function findContext(query) {
  try {
    const response = await querySudo(query, {}, { sparqlEndpoint: LANDING_ZONE_DATABASE_ENDPOINT, mayRetry: true });
...
export async function deleteFromLandingZoneGraph(statements) {
  ...
  await updateWithRecover(statements, deleteQueryTemplate, LANDING_ZONE_DATABASE_ENDPOINT)
}
...
export async function insertIntoLandingZoneGraph(statements) {
  ...
  await updateWithRecover(statements, insertQueryTemplate, LANDING_ZONE_DATABASE_ENDPOINT)
}
