...
async function findSubjectTypes(subject) {
  try {
    const response = await querySudo(subjectTypeQuery(DCR_LANDING_ZONE_GRAPH, subject), {}, DCR_LANDING_ZONE_DATABASE_ENDPOINT);
    console.log(`type query response: ${JSON.stringify(response)}`);

    if (response.results.bindings.length) {
      return response.results.bindings.map(
        r => ({
          'graph': sparqlEscapeUri(DCR_LANDING_ZONE_GRAPH),
...
export async function deleteFromLandingZoneGraph(statements) {
  ...
  return await retryPromiseWithDelay(
    updateSudo(deleteQueryTemplate(statements), {}, DCR_LANDING_ZONE_DATABASE_ENDPOINT)
  );
}
...
export async function insertIntoLandingZoneGraph(statements) {
  ...
  return await retryPromiseWithDelay(
    updateSudo(insertQueryTemplate(statements), {}, DCR_LANDING_ZONE_DATABASE_ENDPOINT)
  );
}
