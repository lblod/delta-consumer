...
import {
  DRC_LANDING_ZONE_DATABASE_ENDPOINT,
  DRC_LANDING_ZONE_GRAPH
} from '../config';
import {
  RDF_TYPE_URI,
  RDF_TYPE
...
async function findSubjectTypes(subject) {
  try {
    const response = await querySudo(subjectTypeQuery(DRC_LANDING_ZONE_GRAPH, subject), {}, DRC_LANDING_ZONE_DATABASE_ENDPOINT);
    console.log(`type query response: ${JSON.stringify(response)}`);

    if (response.results.bindings.length) {
      return response.results.bindings.map(
        r => ({
          'graph': sparqlEscapeUri(DRC_LANDING_ZONE_GRAPH),
...
export async function deleteFromLandingZoneGraph(statements) {
  ...
  return await retryPromiseWithDelay(
    updateSudo(deleteQueryTemplate(statements), {}, DRC_LANDING_ZONE_DATABASE_ENDPOINT)
  );
}
...
export async function insertIntoLandingZoneGraph(statements) {
  ...
  return await retryPromiseWithDelay(
    updateSudo(insertQueryTemplate(statements), {}, DRC_LANDING_ZONE_DATABASE_ENDPOINT)
  );
}
