...
async function findSubjectTypes(subject) {
  try {
    const response = await querySudo(subjectTypeQuery(LANDING_ZONE_GRAPH, subject), {}, LANDING_ZONE_DATABASE_ENDPOINT);
...
async function findContext(query) {
  try {
    const response = await querySudo(query, {}, LANDING_ZONE_DATABASE_ENDPOINT);
...
export async function deleteFromLandingZoneGraph(statements) {
  ...
  return await retryPromiseWithDelay(
    updateSudo(deleteQueryTemplate(statements), {}, LANDING_ZONE_DATABASE_ENDPOINT)
  );
}
...
export async function insertIntoLandingZoneGraph(statements) {
  ...
  return await retryPromiseWithDelay(
    updateSudo(insertQueryTemplate(statements), {}, LANDING_ZONE_DATABASE_ENDPOINT)
  );
}
