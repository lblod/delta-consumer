...
async function findSubjectTypes(subject) {
  try {
    const response = await querySudo(subjectTypeQuery(INGEST_GRAPH, subject), {}, DCR_INGEST_DATABASE_ENDPOINT);
...
export async function deleteFromIngestGraph(statements) {
  ...
  return await retryPromiseWithDelay(
    updateSudo(deleteQueryTemplate(statements), {}, DCR_INGEST_DATABASE_ENDPOINT)
  );
}
...
export async function insertIntoIngestGraph(statements) {
  ...
  return await retryPromiseWithDelay(
    updateSudo(insertQueryTemplate(statements), {}, DCR_INGEST_DATABASE_ENDPOINT)
  );
}
