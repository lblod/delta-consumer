...
import { LANDING_ZONE_DATABASE_ENDPOINT, LANDING_ZONE_GRAPH } from './../config';
...
async function triggerOnNewRDFTypeStatements(
    ...
    if (newRDFTypeStatements.length) {
        const subjects = [
            ...new Set(
                newRDFTypeStatements.map((statement) => statement.subject)
            ),
        ];
...
export async function deleteFromLandingZoneGraph(statements) {
    return await deleteFromGraph(statements, LANDING_ZONE_DATABASE_ENDPOINT, LANDING_ZONE_GRAPH);
}

export async function insertIntoLandingZoneGraph(statements) {
    return await insertIntoGraph(statements, LANDING_ZONE_DATABASE_ENDPOINT, LANDING_ZONE_GRAPH);
}
