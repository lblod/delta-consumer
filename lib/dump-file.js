...
import { remapTriplesInDeltas } from './delta-sparql-mapping';
...
class DumpFile {
  ...
  async toTermObjectAndDispatch(data, dispatch) {
    ...
    if(ENABLE_TRIPLE_REMAPPING) {
      triples = data.map(triple => {
        return {
          graph: null,
          subject: this.rdfJSTermToSparqlJson(triple.subject),
          predicate: this.rdfJSTermToSparqlJson(triple.predicate),
          object: this.rdfJSTermToSparqlJson(triple.object)
        };
      });
      triples = await remapTriplesInDeltas([{ inserts: triples, deletes: [] }]);
      triples = triples.length ? triples[0].inserts : [];
      triples = toTermObjectArray(triples);
    }
  ...
  rdfJSTermToSparqlJson(rdfTerm) {
    ...
    if (termType === 'NamedNode') {
      ...
      // we ignore the xsd:string on ingest
      if (datatype && datatype.value && datatype.value != 'http://www.w3.org/2001/XMLSchema#string')
        return { type: "literal", value, datatype: datatype.value };
      else if (language)
        return { type: "literal", value, datatype: datatype.value, 'xml:lang': language };
      else
