...
import loggedInFetch from './fetcher';
...
class DumpFile {
  ...
  async download() {
    try {
      await loggedInFetch(this.downloadUrl)
...
export async function getLatestDumpFile() {
  try {
    ...
    const responseDataset = await loggedInFetch(
    ...
    if (dataset.data.length) {
      ...
      const resultDistribution = await loggedInFetch(`${distributionUri}?include=subject`, {
        headers: {
          'Accept': 'application/vnd.api+json'
        }
      });
