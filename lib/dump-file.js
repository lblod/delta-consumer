...
class DumpFile {
    ...
    return new Promise((resolve, reject) => {
      ...
      const quads = parser.parse(data, (error, quad) => {
        if(error) {
          ...
          //TODO: check if this is still needed, but I remeber there were subtle differences
          try {
            triples.push(
              {
                graph: null,
  ...
  async tryParseStringChunk(stringData) {
    if(stringData) {
      try {
        return await this.toTermObjectArray(stringData);
      }
      catch(e){
        debugger;
        //console.log();
