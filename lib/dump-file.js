...
export async function getLatestDumpFile() {
    try {
        ...
        if (dataset.data.length) {
            ...
            try {
                // some producers don't exposer their file endpoint. in that case we just rollback to random ttl filename
                const fileResponse = await fetcher(
                    `${GET_FILE_ENDPOINT.replace(
                        ':id',
                        selected_distribution.relationships.subject.data.id
                    )}`,
                    {
                        headers: {
                            Accept: 'application/vnd.api+json',
                ...
                file = {
                    data: {
                        format: 'text/turtle',
                        name: mu.uuid() + '.ttl',
                        ...selected_distribution.relationships.subject.data,
                    },
                };
            }
