import * as muAuthSudo from './sparql';
...
import fetcher, { fetchToFile } from './fetcher';
...
class DumpFile {
  ...
  async download() {
    try {
      await fetchToFile(this.downloadUrl, {}, this.filePath);
    } catch (e) {
  ...
  async parseAndDispatch(dispatch) {
    ...
    return new Promise((resolve, reject) => {
      ...
      streamParser.on('end', async function () {
        // Store remaining triples. Also resolve when there are none left:
        // not settling here would hang the initial sync forever whenever the
        // number of quads in the file is an exact multiple of BATCH_SIZE.
        try {
