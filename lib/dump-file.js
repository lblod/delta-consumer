...
class DumpFile {
  constructor(distributionData, data) {
    ...
    this.compression = distributionData["compression"];
    this.extension = distributionData["compression"] ? '.ttl.gz' : '.ttl';
  }
  ...
  get filePath() {
    return path.join(BASEPATH,`${this.id}${this.extension}`);
  }
  ...
  async load() {
    try {
      ...
      const ttlStream = this.compression ? fs.createReadStream(this.filePath).pipe(zlib.createGunzip()) : fs.createReadStream(this.filePath);
...
export async function getLatestDumpFile() {
  try {
    ...
    if (dataset.data.length) {
      ...
      const distributionMetaData = distribution.data[0].attributes;
