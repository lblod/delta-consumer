  ...
  async load(onBatchReady) {
    try {
      ...
      for await (const line of rl) {
        ...
        if(batchedLines.length == CHUNK_SIZE) {
          const batchedTriples = await this.toTermObjectArray(batchedLines.join(' '));
          await onBatchReady(triples);
          triples.push(...batchedTriples);
          batchedLines = [];
        }
      }
