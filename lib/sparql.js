 ...
 */
// The name ends up in the persisted oslc:message, so alerting can match on it.
...
function withTimeout(promise, queryString, connectionOptions) {
  ...
  const guard = new Promise((_, reject) => {
    timer = setTimeout(() => {
      ...
      reject(new SparqlTimeoutError(`SPARQL request to ${endpoint} timed out after ${SPARQL_TIMEOUT_MS} ms (see DCR_SPARQL_TIMEOUT_MS). Query: ${summary}`));
    }, SPARQL_TIMEOUT_MS);
  });
