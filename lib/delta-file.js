...
export default class DeltaFile {
  ...
  async load() {
    try {
      ...
      let termObjectChangeSetsWithContext = addContext(termObjectChangeSets)
...
function compileContextConfiguration(contextConfiguration) {
  ...
  // console.log(`compiled config:\n${compiledConfig.contextQueries[0].query('')}`)
  return contextConfig;
}

function addContext(changeSets) {
  let changeSetsWithTypes = addTypes(changeSets);
  const { contextQueries } = contextConfig;

  contextQueries.forEach(
    ({ trigger, query }) => {
      const { subjectType, predicateValue } = trigger;

      changeSetsWithTypes.forEach(
        (changeSet) => {
          let { inserts, deletes } = changeSet; //TODO: add context to deletes as well
          inserts.filter(
            (insert) => {
              (
                !subjectType ||
                (
                  insert.predicate === RDF_TYPE_URI &&
                  insert.object === subjectType
                )
              ) &&
                (
                  !predicateValue ||
                  (
                    insert.predicate === predicateValue
                  )
                )
            }
          ).forEach(statementWhichNeedsContext => {
            const { subject } = statementWhichNeedsContext;
            const contextQuery = query(subject);
            const context = findContext(contextQuery);
            inserts.push(...context);
            // statementWhichNeedsContext.context = context;
          });
          deletes.filter(
            (delete_) => {
              (
                !subjectType ||
                (
                  delete_.predicate === RDF_TYPE_URI &&
                  delete_.object === subjectType
                )
              ) &&
                (
                  !predicateValue ||
                  (
                    delete_.predicate === predicateValue
                  )
                )
            }
          ).forEach(statementWhichNeedsContext => {
...
function addTypes(changeSets) {
  ...
  if (exhaustive) {
    for (const changeSet of changeSets) {
      ...
      inserts = exhaustiveAddTypes(inserts, typeCache);
      deletes = exhaustiveAddTypes(deletes, typeCache);
    }
...
async function findSubjectTypes(subject) {
  return await querySudo(subjectTypeQuery(INGEST_GRAPH, subject), {}, INGEST_DATABASE_ENDPOINT)
...
async function findContext(query) {
  return await querySudo(query, {}, INGEST_DATABASE_ENDPOINT)
    .then(function (response) {
      console.log(`context query response: ${JSON.stringify(response)}`)
      return response.results.bindings.map(
        r => ({
          'graph': INGEST_GRAPH,
          'subject': sparqlEscapeUri(r.s.value),
          'predicate': sparqlEscapeUri(r.p.value),
          'object': sparqlEscapeUri(r.o.value) // TODO add literal support
        })
      )
    })
    .catch(function (err) {
