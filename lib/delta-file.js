...
async function addContext(changeSets) {
  ...
  for (let { trigger, queryTemplate } of contextQueries) {
    for (let { inserts, deletes } of changeSetsWithTypes) {

      // console.log(`trigger: ${JSON.stringify(trigger, null, 2)}`)
      // console.log(`inserts: ${JSON.stringify(inserts, null, 2)}`)

      const insertsNeedingContext = inserts.filter(contextTriggerFilter(trigger));
      ...
      const deletesNeedingContext = deletes.filter(contextTriggerFilter(trigger));
