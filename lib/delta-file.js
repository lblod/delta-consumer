...
async function addContext(changeSets) {
  let changeSetsWithTypes = await addTypes(changeSets);
...
async function addTypes(changeSets) {
  ...
  if (exhaustive) {
    ...
    for (let changeSet of changeSets) {
      ...
      inserts = await nonExhaustiveAddTypes(inserts, typeCache);
      // find subjects in the deletes
      deletes = await nonExhaustiveAddTypes(deletes, typeCache);
    }
  }
  return changeSets;
}
