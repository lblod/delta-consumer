...
import {
  DELTA_FILE_FOLDER, DOWNLOAD_FILE_ENDPOINT, KEEP_DELTA_FILES, SYNC_BASE_URL, SYNC_LOGIN_ENDPOINT
} from '../config';
...
export default class DeltaFile {
  ...
  async download() {
    try {
      ...
      await fetch(this.downloadUrl, { headers: { cookie: this.nextCookie } })
        .then(res => new Promise((resolve, reject) => {
          this.nextCookie = res.headers.raw()['set-cookie'] ? res.headers.raw()['set-cookie'] : this.nextCookie;
