...
import {
  DELTA_FILE_FOLDER, DOWNLOAD_FILE_ENDPOINT, KEEP_DELTA_FILES, ENABLE_DELTA_CONTEXT
} from '../config';
...
export default class DeltaFile {
  ...
  async removeFile(){
    const directory = path.join(DELTA_FILE_FOLDER, this.folderByDay);
    let fullPath = path.join(directory, this.fileName);
    if (fs.pathExists(fullPath)){
      fs.remove(fullPath)
    } else if (KEEP_DELTA_FILES) {
      console.log(`Cleanup: unable to find file with path ${fullPath}`);
    }
  }
}
