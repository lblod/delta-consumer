...
Extendable consumer to sync data from external sources based on diff files generated by a producer. You can find an example producer [here](http://github.com/lblod/mandatendatabank-mandatarissen-producer).
...
By default, the delta-consumer will use the fetched information to maintain an ingest graph. It implements the same behavior as [delta-consumer-single-graph-maintainer](https://github.com/lblod/delta-consumer-single-graph-maintainer), which is deprecated by now.

However, custom ingestion rules are perfectly possible. Read along if you want to know how you can achieve this. 'Triples-dispatching' is the term we will use when moving triples to the correct place.
