...
export async function batchedDbUpdate(
    muUpdate,
    graph,
    triples,
    extraHeaders,
    ...
    for (let i = 0; i < triples.length; i += batchSize) {
        ...
        console.log(
            `Sleeping before next query execution: ${sleepBetweenBatches}`
        );
...
async function operationWithRetry(
    ...
    try {
        ...
        if (attempt >= maxAttempts) {
            console.log(
                `Max attempts reached for ${callback.toString()}, giving up`
            );
            throw e;
        }
...
function mainConversion(fetch, triples) {
    ...
    return fetch('http://reasoner/reason/example', requestOptions).then(
...
export function transformStatements(fetch, triples) {
    ...
    return transformTriples(fetch, triples.join('\n')).then((graph) => {
        ...
        console.log(
            `CONVERSION: FROM ${triples.length} triples to ${statements.length}`
        );
        return statements;
    });
}
...
export async function transformLandingZoneGraph(fetch, constants) {
    ...
    const response = await fetch(
        `http://reasoner/reason/op2dl/main?data=${encodeURIComponent(
            `${LANDING_ZONE_DATABASE_ENDPOINT}?default-graph-uri=&query=CONSTRUCT+%7B%0D%0A%3Fs+%3Fp+%3Fo%0D%0A%7D+WHERE+%7B%0D%0A+GRAPH+<${LANDING_ZONE_GRAPH}>+%7B%0D%0A%3Fs+%3Fp+%3Fo%0D%0A%7D%0D%0A%7D&should-sponge=&format=text%2Fplain&timeout=0&run=+Run+Query`
        )}`
    );
