...
async function dispatch(lib, data) {
    ...
    for (let { original, withContext } of zippedChangeSets) {
        let originalDeletes,
            ...
            deletesWithContext;
        try {
            originalInserts = original.inserts.map(
                (o) => `${o.subject} ${o.predicate} ${o.object}.`
            );
            originalDeletes = original.deletes.map(
                (o) => `${o.subject} ${o.predicate} ${o.object}.`
            );
        } catch (e) {
        ...
        try {
            insertsWithContext = withContext.inserts.map(
                (o) => `${o.subject} ${o.predicate} ${o.object}.`
            );
            deletesWithContext = withContext.deletes.map(
                (o) => `${o.subject} ${o.predicate} ${o.object}.`
            );
        } catch (e) {
            console.log(`Error while processing change set with context: ${e}`);
            console.log(
                `Change set with context: ${JSON.stringify(withContext)}`
            );
            throw e;
        }
        ...
        if (originalDeletes.length) {
            ...
            const transformedDeletes = await transformStatements(
                fetch,
                deletesWithContext
            );
        ...
        if (originalInserts.length) {
            ...
            const transformedInserts = await transformStatements(
                fetch,
                insertsWithContext
            );
